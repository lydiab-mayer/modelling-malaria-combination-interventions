...
experiments <- c("Obj6_Scen1_PreEryth_RTSS", "Obj6_Scen1_LayerCounterfactual_PreEryth_RTSS")
...
load_dfs <- function(file) {
  df <- read.table(file, sep = "\t", header = TRUE, as.is = TRUE, stringsAsFactors = FALSE)
  return(df)
}
...
for (exp in experiments) {
  ...
  # Load unaggregated postprocessing data
  files <- Sys.glob(paste0(GROUP_dr, exp, "/postprocessing/seeds*"))
  dfs <- lapply(files, load_dfs)
  df <- do.call(rbind, dfs)
  
  # Extract desired outcomes
  dfOut[[exp]] <- df %>%
    select(c(rownames(param_ranges_cont)), "Seasonality", "EIR", "Access", "seed", starts_with(c(paste0("Baseline_", id), paste0("Counterfactual_", id), paste0("Intervention_", id))))
  ...
  dfOutAggregate[[exp]] <- dfOut[[exp]] %>%
    group_by(across(-c(Value, seed, Group))) %>%
...
# Aggregate over intervention
dfOutAggregate <- dfOutAggregate %>%
  group_by(across(-c(Experiment, medianValue, minValue, maxValue))) %>%
  summarise(medianValue = mean(medianValue),
...
for (index in 1:2) {
  param_label[[index]] <- paste0("Imperfect deployment scenario | ",
...
saveRDS(dfOut, "./data_and_figures/supplement_fig13/data_fig13.rds")
saveRDS(dfOutAggregate, "./data_and_figures/supplement_fig13/data_aggregate_fig13.rds")
saveRDS(param_label, "./data_and_figures/supplement_fig13/label_fig13.rds")
