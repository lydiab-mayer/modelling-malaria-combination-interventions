...
# Read in data
dfOut <- readRDS("./data_and_figures/supplement_fig13/data_fig13.rds")
dfOutAggregate <- readRDS("./data_and_figures/supplement_fig13/data_aggregate_fig13.rds")
...
# Read in plot label
param_label <- readRDS("./data_and_figures/supplement_fig13/label_fig13.rds")
param_label
...
ggsave(filename = paste0("./data_and_figures/supplement_fig13/fig13.jpg"),
       plot = last_plot(),
       width = 8,
       height = 5.5,
       dpi = 400)
...
# Extract cumulative cases at 5 and 10 years
pointEstimates <- dfOutAggregate %>%
  filter(Intervention %in% c("SMC", "Pre-liver stage therapeutic", "SMC + pre-liver stage therapeutic"),
         AgeGroup %in% c(5, 10)) %>%
  select(-c(minValue, maxValue)) %>%
  pivot_wider(names_from = Intervention,
              values_from = medianValue)

# Calculate impact relative to each counterfactual scenario
pointEstimates <- pointEstimates %>%
  mutate(impactSMC = (SMC - `SMC + pre-liver stage therapeutic`) / SMC,
         impactLiverStage = (`Pre-liver stage therapeutic` - `SMC + pre-liver stage therapeutic`) / `Pre-liver stage therapeutic`)
