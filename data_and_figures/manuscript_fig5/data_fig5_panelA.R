...
df <- df %>%
  mutate(Seasonality = case_match(Seasonality,
         ...
         OutcomeLabel = case_match(OutcomeLabel,
                              "Reduction_CumCPPY_age5" ~ "Reduction in cumulative uncomplicated cases\nby 5 years old",
                              "Reduction_SevCumCPPY_age5" ~ "Reduction in cumulative severe cases\nby 5 years old",
                              "Reduction_CumCPPY_age10" ~ "Reduction in cumulative uncomplicated cases\nby 10 years old",
                              "Reduction_SevCumCPPY_age10" ~ "Reduction in cumulative severe cases\nby 10 years old"),
  ...
  mutate(parameter = factor(parameter, levels = c("Pre-liver stage initial efficacy [30 - 100%]",
         ...
         OutcomeLabel = factor(OutcomeLabel, levels = c("Reduction in cumulative uncomplicated cases\nby 5 years old",
                                                        "Reduction in cumulative uncomplicated cases\nby 10 years old",
...
saveRDS(df, "./data_and_figures/manuscript_fig5/data_fig5_panelA.rds")
