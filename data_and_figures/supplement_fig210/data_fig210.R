...
experiments <- c("Obj6_Scen4_PreEryth", "Obj6_Scen4_BloodStage", "Obj6_Scen4_PreEryth_BloodStage")
...
for (exp in experiments) {
  ...
  df <- df %>%
    separate(col = scenario, 
             into = c("Experiment", "Seasonality", "System", "EIR", "Agegroup", "Decay", "Access", "Temp", "Outcome", "Age"),
             sep = "_",
             remove = FALSE)
...
data <- data %>%
  mutate(Seasonality = case_match(Seasonality,
         ...
         Experiment = case_match(Experiment,
                                 "Obj6Scen4PreEryth" ~ "Pre-erythrocytic activity alone",
                                 "Obj6Scen4BloodStage" ~ "Blood stage activity alone",
                                 "Obj6Scen4PreErythBloodStage" ~ "Pre-erythrocytic and blood stage activity"),
...
saveRDS(data, "/scicore/home/penny/brauna0000/M3TPP/data_and_figures/supplement_fig210/data_fig210.rds")
