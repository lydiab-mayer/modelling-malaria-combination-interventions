...
source(paste0("./Experiments/iTPP3_tradeoffs/convert_access.R"))

# Insert experiment name here
exp ="iTPP3_tradeoffs_5rounds"
...
######################################
 ...
 QOS = "30min" # options: 30min; 6hours; 1day
 ...
 # EIR
 EIR = data.frame(EIR = c(2, 4, 8, 16, 32, 64, 128))
 ...
 MaxAge = data.frame(MaxAge = c(5, 10), maxGroup = c(8, 13)) # age groups specified as per iTPP3 document
 
 # Intervention decay
 Decay <- data.frame(fundecay = c("weibull"), kdecay = c(5.395333), Decay = c("exp")) # fundecay and kdecay taken from scaffold_smc xml, parameterised in Burgert et al 2021
...
2*nrow(Seasonality)*nrow(Biting_pattern)*nrow(EIR)*nrow(Decay)*nrow(MaxAge)*nrow(Access)*noSamples*noSeeds
