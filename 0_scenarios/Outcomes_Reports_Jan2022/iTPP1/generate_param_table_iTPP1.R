...
# Simple function without sampling
gen_paramtable_simple <- function(exp, param_cat, noSeeds, chunk_size) {
...
# Created by Narimane Nekkab to remove EIR, access, and seasonality combinations to reduce number of sims
gen_paramtable_optimizedCombos <- function(exp, param_ranges_cont,param_cat, noSamples, noSeeds, chunk_size, EIR_ref) {
  ...
  # Column order
  param_tab_columns = colnames(param_tab_all)
  
  # Seasonal levels
  seasonal_profiles=levels(param_tab_all$Seasonality)
  param_tab_all$Seasonality = as.character(param_tab_all$Seasonality)
  
  # Column from ref
  columns_ref = colnames(EIR_ref)
  EIR_ref$Seasonality = as.character(EIR_ref$Seasonality)
  
  # Match
  matched_ref = merge(EIR_ref, param_tab_all, by = columns_ref, all.x = T)
  
  # Reorder columns 
  matched_ref = matched_ref[,param_tab_columns]
  
  # Remove duplicates
  matched_ref = matched_ref %>% dplyr::distinct()
  
  # Replace
  param_tab_all = matched_ref
...
# Created by Narimane Nekkab to remove EIR, access, and seasonality combinations to reduce number of sims
gen_paramtable_optimizedCombos_catonly <- function(exp, param_cat, noSamples, noSeeds, chunk_size, EIR_ref) {
  ...
  # Remove na
  param_tab_all = param_tab_all[complete.cases(param_tab_all),]
